...
public class MesBydlabelprintController extends BaseController
{
     ...
     */
    //@PreAuthorize("@ss.hasPermi('extra:bydlabelprint&scan:list')")
    @GetMapping("/unscanned/list")
    public TableDataInfo unscannedList(MesBydlabelprint mesBydlabelprint)
    {
        startPage();
        List<MesBydlabelprint> list = mesBydlabelprintService.getMesBydLabelPrintUnscannedList(mesBydlabelprint);
        return getDataTable(list);
    }
     ...
     * 导出未扫码标签列表列表
     */
    @Log(title = "bydunscannedlabelprint", businessType = BusinessType.EXPORT)
    @PostMapping("/unscanned/export")
    public void bydUnscannedLabelPrintExport(HttpServletResponse response, MesBydlabelprint mesBydlabelprint)
    {
        List<MesBydlabelprint> list = mesBydlabelprintService.getMesBydLabelPrintUnscannedList(mesBydlabelprint);
        ExcelUtil<MesBydlabelprint> util = new ExcelUtil<MesBydlabelprint>(MesBydlabelprint.class);
        util.exportExcel(response, list, "bydunscannedlabelprint数据");
    }

    /**
     * 查询bydlabelprintscan列表
     */
    @GetMapping("/printScan/list")
    public TableDataInfo list(MesBydLabelPrintScan mesBydLabelPrintScan)
    {
        startPage();
        List<MesBydLabelPrintScan> list = mesBydlabelprintService.selectMesBydLabelPrintScanList(mesBydLabelPrintScan);
        return getDataTable(list);
    }

    /**
     * 导出bydlabelprintscan列表
     */
    //@PreAuthorize("@ss.hasPermi('extra:bydlabelprint&scan:export')")
    @Log(title = "bydlabelprintscan", businessType = BusinessType.EXPORT)
    @PostMapping("/printScan/export")
    public void export(HttpServletResponse response, MesBydLabelPrintScan mesBydLabelPrintScan)
    {
        List<MesBydLabelPrintScan> list = mesBydlabelprintService.selectMesBydLabelPrintScanList(mesBydLabelPrintScan);
        ExcelUtil<MesBydLabelPrintScan> util = new ExcelUtil<MesBydLabelPrintScan>(MesBydLabelPrintScan.class);
        util.exportExcel(response, list, "bydlabelprintscan数据");
    }

    /**
     * 获取bydlabelprintscan数量统计
     */
    @GetMapping("/printScan/count")
    public AjaxResult Count(MesBydlabelprint mesBydlabelprint)
    {
        return success(mesBydlabelprintService.getMesBydLabelPrintScanCount(mesBydlabelprint));
    }
